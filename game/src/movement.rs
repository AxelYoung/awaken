    ...
    |id, moveable: &mut Moveable, position: &mut Position, cell: &mut Cell| {
        if moveable.moving {
            ...
            if moveable.accumulator >= moveable.duration {
                ...
                let x = start_pos.x as f32 + ((end_pos.x - start_pos.x) as f32 * (moveable.accumulator as f32 / moveable.duration as f32));
                let y = start_pos.y as f32 + ((end_pos.y - start_pos.y) as f32 * (moveable.accumulator as f32 / moveable.duration as f32));
